...
const (
	iscDastGcsSecretPath = "/var/run/vault/rapidast-sa-isc/isc-dast-gcs-secret"
)
...
func RunRapidASTScan(f *Framework, namespace string) error {
	...
	// Check if GCS secret file exists for upload support
	uploadCentralStorage := false
	_, err = os.ReadFile(iscDastGcsSecretPath)
	if err == nil {
		uploadCentralStorage = true
	}

	// Read the rapidast config template
	testDataDir := filepath.Join(f.projectRoot, "tests", "data", "rapidast")
	configFileName := "rapidast-config.yaml"
	if uploadCentralStorage {
		configFileName = "rapidast-config-upload.yaml"
	}
	configTemplatePath := filepath.Join(testDataDir, configFileName)
	...
	// Include GCS secret if upload is enabled
	if uploadCentralStorage {
		gcsSecretData, err := os.ReadFile(iscDastGcsSecretPath)
		if err != nil {
			return fmt.Errorf("failed to read GCS secret file: %w", err)
		}
		configMap.BinaryData["dast-gcs-secret.json"] = gcsSecretData
		log.Printf("Including GCS secret for result upload")
	}
