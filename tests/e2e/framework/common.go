...
func (f *Framework) WaitForScanStatus(namespace, name string, targetStatus compv1alpha1.ComplianceScanStatusPhase) error {
	...
	timeoutErr := wait.Poll(RetryInterval, Timeout, func() (bool, error) {
		...
		case compv1alpha1.PhaseDone:
			// assert exampleComplianceScan.Annotations[compv1alpha1.ComplianceCheckCountAnnotation] is set
			if exampleComplianceScan.Annotations[compv1alpha1.ComplianceCheckCountAnnotation] == "" {
				return true, fmt.Errorf("compliancescan %s should have checkCount annotation", name)
			}
		}
