...
// TestOperatorUpgradeResourcesAndMCPStability validates that compliance operator resources
// and MachineConfigPool status remain stable during operator upgrades.
// This test provides upstream parity for downstream tests OCP-45014 and OCP-45956.
//
// The test validates that when the operator is upgraded:
// 1. No resources (rules, variables, profiles) are lost
...
// - Environment variable UPGRADE_CATALOG_SOURCE_IMAGE pointing to newer operator version
// - Environment variable STARTING_CSV with the older version to install first (e.g., "compliance-operator.v1.6.2")
// - Or, skip the test if upgrade testing is not configured
