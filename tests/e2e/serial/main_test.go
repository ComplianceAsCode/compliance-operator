...
func TestComplianceCheckResultLabels(t *testing.T) {
	f := framework.Global
	suiteName := "test-check-result-labels"
	scanName := fmt.Sprintf("%s-workers-scan", suiteName)
	selectWorkers := map[string]string{
		"node-role.kubernetes.io/worker": "",
	}
	...
	// Verify compliance scan result compliancecheckresult through label
	checkName := fmt.Sprintf("%s-no-netrc-files", scanName)

	// Check by severity label
	err = f.AssertCheckResultByLabel(f.OperatorNamespace, compv1alpha1.ComplianceCheckResultSeverityLabel, "medium", checkName)
	if err != nil {
		t.Fatal(err)
	}

	// Check by status label
	err = f.AssertCheckResultByLabel(f.OperatorNamespace, compv1alpha1.ComplianceCheckResultStatusLabel, string(compv1alpha1.CheckResultPass), checkName)
	if err != nil {
		t.Fatal(err)
	}

	// Check by scan name label
	err = f.AssertCheckResultByLabel(f.OperatorNamespace, compv1alpha1.ComplianceScanLabel, scanName, checkName)
	if err != nil {
		t.Fatal(err)
	}

	// Check by suite label
	err = f.AssertCheckResultByLabel(f.OperatorNamespace, compv1alpha1.SuiteLabel, suiteName, checkName)
	if err != nil {
		t.Fatal(err)
	}
}
