...
func TestResultServerNodeSelectorMaster(t *testing.T) {
	...
	if err := f.WaitForSuiteScansStatus(f.OperatorNamespace, b, compv1alpha1.PhaseDone, compv1alpha1.ResultNonCompliant); err != nil {
		if err2 := f.WaitForSuiteScansStatus(f.OperatorNamespace, b, compv1alpha1.PhaseDone, compv1alpha1.ResultCompliant); err2 != nil {
			t.Fatalf("ComplianceSuite %s did not complete: %v", b, err2)
		}
	}
}
