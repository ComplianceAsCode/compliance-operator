...
func TestTailoringEnabledRulesGenerateRemediations(t *testing.T) {
	...
	// Create a TailoredProfile with the rule as an enabled rule (not manual)
	enabledTPName := framework.GetObjNameFromTest(t)
	enabledWorkerScanName := fmt.Sprintf("%s-worker", enabledTPName)
	...
	enabledCheckName := fmt.Sprintf("%s-kubelet-eviction-thresholds-set-soft-imagefs-available", enabledWorkerScanName)
	...
	// The status should NOT be MANUAL for enabled rules
	if actualCheck.Status == compv1alpha1.CheckResultManual {
		t.Fatalf("Expected check result status to be FAIL for enabled rule, but got MANUAL")
	}
	...
	enabledWithLabel := client.MatchingLabels{
		compv1alpha1.ComplianceScanLabel: enabledWorkerScanName,
	}
	...
	expectedRemediationName := fmt.Sprintf("%s-kubelet-eviction-thresholds-set-soft-imagefs-available", enabledWorkerScanName)
