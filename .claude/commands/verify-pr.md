...
3. **Operator image**: Ask the user to choose:
   - **Build from current branch**: Build locally with `make image` and push to a registry.
     - Ask for the registry (e.g. `quay.io/<your-username>`). Confirm they are logged in with `podman login <registry> --get-login`.
     - The image tag should be `pr<number>` (e.g. `quay.io/<your-username>/compliance-operator:pr1263`).
...
1. Run `make image IMG=<registry>/compliance-operator:<tag>` to build the operator image with the correct tag directly (e.g. `make image IMG=quay.io/<your-username>/compliance-operator:pr1263`). No separate `podman tag` step needed.
